                ...
                _data.read('users', phone, (err, userData) => {
                    ...
                    if (!err && userData) {
                        ...
                        if (userChecks.length < config.maxChecks) {
                            //Verify DNS of url resolves or workers can skip it.
                            ...
                            dns.resolve(hostName, (err, records)=>{
                                if (!err && records) {
                                    ...
                                    callback(400, { 'Error': "Hostname did not resolve." })
                                }
                            });
                        } else {
...
handlers._tokens.post = (data, callback) => {
    ...
    if (pass && phone) {
        ...
        _data.read('users', phone, function (err, userData) {
            ...
            if (!err && userData) {
                _performance.mark('Begin pass hash');
                var hashPass = helpers.hash(pass);
                _performance.mark('Complete pass hash');
                ...
                if (hashPass == parse.hashPass) {
                    ...
                    _data.create('tokens', tokenId, tokenObject, function (err) {
                        _performance.mark('Complete storing token');
                        _performance.measure('Beginning to end','Entered function','Complete storing token');
                        ...
                        _performance.measure('Password hashing','Begin pass hash','Complete pass hash');
                        _performance.measure('Token data creation','Creating data for token','Begin storing token');
                        _performance.measure('Token storing','Begin storing token', 'Complete storing token');

                        let measure = _performance.getEntriesByType('measure');
                        measure.forEach(function(measurement){
